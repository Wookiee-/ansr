...
void do_prompt(CHAR_DATA *ch, char *argument)
{
   ...
   if( !strcmp( argument, "all" ) )
      strcpy( buf, "<{D%h{chp {D%m{cm {D%v{cmv{x> ");
   else
   {
...
void do_exits (CHAR_DATA * ch, char *argument)
{
    ...
    found = FALSE;
    for (door = 0; door <= 5; door++)   /* please change the 9 to 5 if youdont have extra exits */
    {
        if ((pexit = ch->in_room->exit[door]) != NULL
...
void do_score (CHAR_DATA * ch, char *argument)
{
    ...
    sprintf(buf, "    {g|{c   Exp:{C %-11ld {g|{c Int:{C %2d{c/{C%2d {g| {cBash:{C %-6d {G+{g-----{G+{x\n\r",
    ...
    sprintf(buf, "    {g|{c You are{C %-13s {g|{c Weight:{C %4ld {c/{C %-7d {g|{x\n\r",
                 calignment, get_carry_weight(ch) / 10, can_carry_w(ch) / 10);
    send_to_char(buf, ch);

    send_to_char("    {G+{g-----------------------{G+{g------------------------{G+{x\n\r", ch);

    sprintf(buf, "    {g|{c Wimpy:{C %d {c Platinum:{C %-26ld {g|{x\n\r",
                 ch->wimpy, ch->platinum);
    send_to_char(buf, ch);
	...
	printf_to_char( ch, "    {g| {cExplored: {C%d{g|{C%d{x {cplaces, {C%.2f%% {cTotal of World.  {g|{x\n\r",rcnt,top_room,percent);
    }
...
void do_who( CHAR_DATA *ch, char *argument )
{
    ...
    output = new_buf();
    outputimm = new_buf();
    add_buf(outputimm,"\n\r{W**********************\n\r*  {DI{cmmortals {DL{cisted{W  *\n\r{W**********************\n\r{x");
    add_buf(output,"{W**********************\n\r*   {DM{cortals {DL{cisted{W   *\n\r{W**********************\n\r{x");
    for ( d = descriptor_list; d != NULL; d = d->next )
    {
        CHAR_DATA *wch;
	char class[MAX_STRING_LENGTH];
	 ...
	 */
	sprintf(buf2, "{D%3d{x", wch->level);
	switch ( wch->level )
	{
	default: break;
            {
		case MAX_LEVEL - 0 :
		    sprintf(buf2, "{DI{cMP{x");
		    break;
		case MAX_LEVEL - 1 :
		    sprintf(buf2, "{DC{cRE{x");
		    break;
		case MAX_LEVEL - 2 :
		    sprintf(buf2, "{DS{cUP{x");
		    break;
		case MAX_LEVEL - 3 :
		    sprintf(buf2, "{DD{cEI{x");
		    break;
		case MAX_LEVEL - 4 :
		    sprintf(buf2, "{DG{cOD{x");
		    break;
		case MAX_LEVEL - 5 :
		    sprintf(buf2, "{DI{cMM{x");
		    break;
		case MAX_LEVEL - 6 :
		    sprintf(buf2, "{DD{cEM{x");
		    break;
		case MAX_LEVEL - 7 :
		    sprintf(buf2, "{DK{cNI{x");
		    break;
		case MAX_LEVEL - 8 :
		    sprintf(buf2, "{DS{cQU{x");
		    break;
            }
	}
	...
	sprintf( buf2, "\n\r{DP{clayers {DO{cnline: {D%d   P{clayer {DH{cigh: {D%d{x\n\r", count, max_on );
	add_buf(output,buf2);
    }
    if (countimm >= 1)
    {
