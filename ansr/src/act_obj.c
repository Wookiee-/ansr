...
BUFFER *get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    && (ch->level <= HERO) ) {
	do_function(ch, &do_wear, obj->name);
    }
...
void do_give( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_OBJ_STAT(obj,ITEM_FORCED)
    && (victim->level <= HERO) ) {
	do_function(victim, &do_wear, obj->name);
    }
    return;
}
...
CHAR_DATA *find_keeper( CHAR_DATA *ch )
{
    ...
    {
	do_function( keeper, &do_say, "{aTwits are not welcome!{x" );
    ...
    {
	do_function( keeper, &do_say, "{aSorry, I am closed. Come back later.{x" );
	return NULL;
    }
    
    if ( time_info.hour > pShop->close_hour )
    {
	do_function( keeper, &do_say, "{aSorry, I am closed. Come back tomorrow.{x" );
	return NULL;
    }
    ...
    {
	do_function( keeper, &do_say, "{aI don't trade with folks I can't see.{x" );
	return NULL;
    }
